...
def get_transcript(video_id: str, asr: bool = True, timestamps: bool = True) -> str:
    """
    Retrieves the transcript for a given YouTube video ID.

    This function acts as a high-level interface to extract a video's transcript.
    It takes a YouTube video ID and then fetches the transcript text.

    Args:
        video_id (str): The unique identifier of the YouTube video (e.g., "dQw4w9WgXcQ").
        asr (bool): If True, fetches the automatically generated (ASR) transcript.
                    If False, attempts to fetch a manually created transcript.
                    Defaults to True.
        timestamps (bool): If True, includes timestamps in the output.
                           If False, returns only the text content. Defaults to True.

    Returns:
        str: A string containing the full transcript of the video. If timestamps
             are enabled, each segment is on a new line, prefixed by its
             timestamp (e.g., "[0:01] Hello"). Otherwise, it's a single
             continuous string. Returns an empty string if the transcript
             cannot be found.

    Raises:
        Exception: If the request to YouTube's API fails or returns a non-200 status code,
                   or if the transcript data cannot be parsed from the response.
    """
    transcript_text = fetch_youtube_transcript_text(video_id, asr=asr, timestamps=timestamps)
    return transcript_text

transcript = FunctionTool(func=get_transcript)